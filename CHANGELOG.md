...
口径修正：安装边界 + 批量安装需显式确认（ClawHub LLM 复核）。

- SKILL.md 边界节新增安装边界：只写用户显式指定的目标，批量安装到全部已知智能体目录必须显式 `--yes`，不做静默多目录写入，符号链接目标一律拒绝。
...
- 版本对齐：package.json / SKILL.md / CHANGELOG / 引擎 VERSION / 测试断言 / README 锚点 = 0.2.7。
- 无功能变更（仅文档与版本同步）。
